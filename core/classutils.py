...
class Node:
    def __init__(self, db, node_type=None, connection_historical_id=None, node_id=None):
        ...
        if node_id:
            ...
            SELECT node_type_id, connection_historical_id  FROM node WHERE id = {node_id} LIMIT 1
            """)
            if not res:
                raise ValueError('No node exists with that id')
            self.node_type, self.connection_historical = res[0]
        else:
            ...
            self._db.insert(f"""
                INSERT INTO node (node_type_id, connection_historical_id)
                    VALUES ({node_type or 'NULL'}, {connection_historical_id or 'NULL'})
            """)
